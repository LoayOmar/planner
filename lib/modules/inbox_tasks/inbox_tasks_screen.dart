...
class InboxTasksScreen extends StatelessWidget {
  @override
    ...
    return BlocConsumer<PlannerCubit, PlannerStates>(
      ...
      builder: (context, state) {
        ...
        return Scaffold(
          ...
          body: PlannerCubit.get(context).tasks.isEmpty
              ? Center(
                  child: Column(
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    String subTask =
                        PlannerCubit.get(context).tasks[index]['subTasks'];
                    ...
                    if(subTask.isNotEmpty) {
                      ...
                      String subTaskBool =
                      PlannerCubit.get(context).tasks[index]['subTasksBool'];
                    ...
                    DateTime date = DateTime.parse(
                        PlannerCubit.get(context).tasks[index]['date']);
                    String time =
                        PlannerCubit.get(context).tasks[index]['time'];
                    time = time.replaceAll('TimeOfDay(', '');
                    time = time.replaceAll(')', '');

                    bool state = PlannerCubit.get(context).tasks[index]['State'] == 'true'
                        ? true
                        : false;
                    ...
                    return buildTaskItem(
                      ...
                      onTapFunction: (){
                        navigateTo(context, EditTaskScreen(
                          ...
                          PlannerCubit.get(context).tasks[index]['id'],
                        ));
                      },
                      ...
                      subTasksTitles: PlannerCubit.get(context).subText[index],
                      index: index,
                      time: TimeOfDay(hour: int.parse(time.split(':')[0]), minute: int.parse(time.split(':')[1])),
                      date: date,
                      haveNote: PlannerCubit.get(context).tasks[index]['note'] != '',
                    );
                  },
                  ...
                  itemCount: PlannerCubit.get(context).tasks.length,
                ),
        );
      },
    );
